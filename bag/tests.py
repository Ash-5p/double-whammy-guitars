...
class BagViewsTest(TestCase):
    ...
    def test_add_to_bag_new_item(self):
        response = self.client.post(reverse('add_to_bag', args=[self.item_id]), {
            'quantity': 2,
    ...
    def test_adjust_bag_quantity(self):
        ...
        session.save()
        response = self.client.post(reverse('adjust_bag', args=[self.item_id]), {
            'quantity': 5
        })
        session = self.client.session
    ...
    def test_remove_from_bag(self):
        ...
        session['bag'] = {self.item_id: 2}
        session.save()
        response = self.client.post(reverse('remove_from_bag', args=[self.item_id]))
        session = self.client.session
        self.assertNotIn(self.item_id, session['bag'])
        self.assertEqual(response.status_code, 200)
    ...
    def test_remove_from_bag_error_handling(self):
        response = self.client.post(reverse('remove_from_bag', args=[self.item_id]))
        # Item isn't in session['bag'], so .pop(item_id) should raise KeyError
